...
from app.utils.db import get_dynamic_config, get_robust_db
...
def call_robust_api(namespace, method, payload):
    ...
    try:
        ...
        current_app.logger.error(
            f"Robust API call failed for {method} at {full_url}: {e}"
        )
        return None
...
def create_robust_user(first_name, last_name, password, email):
    ...
    if response_text and "<PrincipalID>" in response_text:
        ...
        if match:
            log_audit_action(
                "User created",
                f"Created user '{first_name}' '{last_name}' with Email '{email}'",
...
def set_user_level(uuid, level):
    ...
    payload = {"PrincipalID": uuid, "UserLevel": level}
...
def update_robust_name(uuid, first_name, last_name):
    ...
    payload = {"PrincipalID": uuid, "FirstName": first_name, "LastName": last_name}
    response_text = call_robust_api("accounts", "setaccount", payload)
    log_audit_action(
        "User Name", f"Changed to '{first_name}' '{last_name}'", target_uuid=uuid
    )
    return response_text and "true" in response_text.lower()
...
def update_robust_email(uuid, email):
    ...
    payload = {"PrincipalID": uuid, "Email": email}
    response_text = call_robust_api("accounts", "setaccount", payload)
...
def update_user_password(user_uuid, new_password):
    ...
    payload = {"PRINCIPAL": user_uuid, "PASSWORD": new_password}
    log_audit_action("User Password", "Changed", target_uuid=user_uuid)
    ...
    # If it says false or error, it definitely failed; otherwise treat as success
    # (OpenSim often returns empty 200 OKs).
    return not ("false" in rt_lower or "error" in rt_lower)
