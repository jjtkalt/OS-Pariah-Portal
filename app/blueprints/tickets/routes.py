...
from app.utils.notifications import (
    ...
    send_ticket_transcript_email,
)
from app.utils.schema import PERM_DELETE_TICKETS, PERM_STAFF_TICKETS
        ...
        with pariah_conn.cursor() as cursor:
            if is_staff:
                ...
                if status_filter == "All Open":
                    cursor.execute(
                        "SELECT * FROM tickets WHERE status IN %s ORDER BY updated_at DESC",
                        (open_statuses,),
                    )
                    ...
                    cursor.execute(
                        "SELECT * FROM tickets WHERE status = %s ORDER BY updated_at DESC",
                        (status_filter,),
                    )
            else:
                # USER LOGIC
                if status_filter == "All Open":
                    cursor.execute(
                        "SELECT * FROM tickets WHERE user_uuid = %s AND status IN %s ORDER BY updated_at DESC",
                        (user_uuid, open_statuses),
                    )
                elif status_filter == "All Tickets":
                    cursor.execute(
                        "SELECT * FROM tickets WHERE user_uuid = %s ORDER BY updated_at DESC",
                        (user_uuid,),
                    )
                else:
                    cursor.execute(
                        "SELECT * FROM tickets WHERE user_uuid = %s AND status = %s ORDER BY updated_at DESC",
                        (user_uuid, status_filter),
                    )

            tickets = cursor.fetchall()

    return render_template(
        "tickets/index.html", tickets=tickets, current_filter=status_filter
    )
...
def new_ticket():
    if request.method == "POST":
        ...
        if not user_uuid:
            ...
            if not email:
                flash(
                    "An email address is required for guest tickets so we can reply to you.",
                    "error",
                )
                return redirect(url_for("tickets.new_ticket"))
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if (
                    ...
                    and allowed_file(attachment.filename)
                ):
...
def view(ticket_id):
    ...
    with pariah_conn.cursor() as cursor:
        ...
        cursor.execute(
            "SELECT * FROM ticket_attachments WHERE ticket_id = %s", (ticket_id,)
        )
...
def reply_ticket(ticket_id):
    ...
    reply_body = (
        request.form.get("message", "").strip() or request.form.get("body", "").strip()
    )
    attachment = request.files.get("attachment")
    ...
    if (
        not reply_body
        and not (attachment and attachment.filename)
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if is_staff:
                ...
                if not new_status:
                    new_status = (
                        "Completed" if not ticket["user_uuid"] else "Waiting on User"
                    )
            else:
...
def delete_ticket(ticket_id):
    ...
    if (
        not has_permission(PERM_DELETE_TICKETS)
        or get_dynamic_config("allow_ticket_deletion") != "true"
    ):
        flash(
            "Unauthorized: Ticket deletion is disabled globally or you lack clearance.",
            "error",
        )
        return redirect(url_for("tickets.view", ticket_id=ticket_id))
