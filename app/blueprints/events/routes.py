...
from app.utils.auth_helpers import rbac_required, require_active_user, has_permission
from app.utils.schema import (
    PERM_MANAGE_EVENTS, PERM_APPROVE_EVENTS, PERM_DELETE_EVENTS,
    PERM_MANAGE_REGION_EVENTS,
)
from app.utils.audit import log_audit_action
from app.utils.markdown_safe import render_markdown
from app.utils.events import (
    EVENT_TIERS, EVENT_CATEGORIES, TIER_LABELS, CATEGORY_LABELS,
    parse_local_datetime, format_pacific, expand_events_for_range,
    fetch_published_events, get_event_by_id, parse_feed_filters,
    load_subscription_by_token, build_ical_feed, build_rss_feed,
    calendar_month_grid, default_reminder_offsets, new_subscription_token,
    event_form_defaults, utc_now_naive, utc_to_local_parts,
    calendar_week_start, calendar_week_dates, expand_event_occurrences,
    parse_reminder_offsets_from_form, REMINDER_OFFSET_CHOICES,
    occurrence_local_date, local_date_utc_range, local_month_utc_range,
    group_occurrences_by_local_date, grid_today,
)
from app.utils.event_notifications import (
    notify_suggestion_staff, notify_submitter_decision,
...
def _can_manage_event_record(event):
    if has_permission(PERM_MANAGE_EVENTS):
        return True
    if event.get('event_tier') == 'region' and event.get('region_uuid'):
        if has_permission(PERM_MANAGE_REGION_EVENTS) or _user_can_control_region(event['region_uuid']):
            return True
    return False
...
def _parse_form_datetimes(form, prefix=''):
    ...
    elif not all_day and form.get(f'{prefix}time_end') and form.get(f'{prefix}date_start'):
        ends_at = parse_local_datetime(
            form.get(f'{prefix}date_start', ''),
...
def _event_row_from_form(form, created_by_uuid, created_by_name, status='draft'):
    ...
    if form.get('recurrence_until_date'):
        recurrence_until = parse_local_datetime(form.get('recurrence_until_date'), None, all_day=True)
...
def _save_calendar_return(view, year, month, day, tier):
    session['events_calendar_return'] = url_for(
        ...
        view=view, year=year, month=month, day=day,
...
def _calendar_common_context(view, year, month, day, tier, today, is_current_period, month_name):
    return {
        'view': view,
        'year': year,
        'month': month,
...
def _expand_range(range_start, range_end, tier_filter=None):
    show_cancelled = str(get_dynamic_config('calendar_show_cancelled')).lower() == 'true'
...
def calendar_index():
    ...
    if view == 'schedule':
        ...
        ctx = _calendar_common_context(
            view, focus.year, focus.month, focus.day, tier, today, is_current,
            f"Schedule from {focus.strftime('%B %d, %Y')}",
        )
        ctx.update({
            'schedule_grouped': schedule_grouped,
            'prev_year': prev_focus.year, 'prev_month': prev_focus.month, 'prev_day': prev_focus.day,
            'next_year': next_focus.year, 'next_month': next_focus.month, 'next_day': next_focus.day,
        })
    ...
    if view == 'week':
        ...
        ctx.update({
            'week_dates': week_dates, 'week_start': week_start, 'by_date': by_date,
            'prev_year': prev_focus.year, 'prev_month': prev_focus.month, 'prev_day': prev_focus.day,
            'next_year': next_focus.year, 'next_month': next_focus.month, 'next_day': next_focus.day,
        })
        _save_calendar_return(view, focus.year, focus.month, focus.day, tier)
    ...
    if view == 'day':
        ...
        ctx.update({
            'day_occurrences': day_occurrences, 'focus_date': focus,
            'prev_year': prev_focus.year, 'prev_month': prev_focus.month, 'prev_day': prev_focus.day,
            'next_year': next_focus.year, 'next_month': next_focus.month, 'next_day': next_focus.day,
        })
        _save_calendar_return(view, focus.year, focus.month, focus.day, tier)
    ...
    ctx = _calendar_common_context(
        view, year, month, day, tier, today, is_current,
...
def search():
    ...
    return render_template(
        'events/search.html', q=q, results=results,
        format_pacific=format_pacific, tier_labels=TIER_LABELS,
...
def detail(event_id):
    ...
    if event.get('recurrence_rule') and event['status'] == 'published':
        now = utc_now_naive()
        upcoming_occurrences = [
            occ for occ in expand_event_occurrences(event, now, now + timedelta(days=90))
    ...
    if request.method == 'POST':
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                """,
                (
                    session['uuid'], event_id, notify_email, notify_inworld,
                    json.dumps(offsets),
                ),
            )
        conn.commit()
    ...
    if request.method == 'POST':
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                """,
                (
                    session['uuid'], session['name'], row['title'], row['description'],
                    row['starts_at'], row['ends_at'], row['all_day'], row['location'],
                    row['slurl'], row['region_uuid'], row['event_tier'], row['category'],
                    row['recurrence_rule'], row['recurrence_until'],
                ),
            )
...
def my_suggestions():
    ...
    return render_template(
        'events/my_suggestions.html', items=items, format_pacific=format_pacific,
        back_url=_calendar_back_url(),
    )
...
def moderation():
    ...
    return render_template(
        'events/moderation.html', pending=pending,
        format_pacific=format_pacific,
...
def approve_suggestion(sid):
    conn = get_pariah_db()
    with conn.cursor() as cursor:
        cursor.execute("SELECT * FROM event_suggestions WHERE id = %s AND status = 'pending'", (sid,))
        sug = cursor.fetchone()
        if not sug:
        ...
        cursor.execute(
            ...
            """,
            (
                sug['title'], sug['description'], sug['starts_at'], sug['ends_at'],
                sug['all_day'], sug['location'], sug['slurl'], sug['region_uuid'],
                sug['submitter_uuid'], sug['event_tier'], sug['category'],
                sug['recurrence_rule'], sug['recurrence_until'], sid,
                session['uuid'], session['name'],
            ),
        )
        cursor.execute(
            """
    ...
    log_audit_action('Event approved', f"Suggestion #{sid} -> published", sug['submitter_uuid'])
...
def manage_event(event_id=None):
    ...
    if event and not _can_manage_event_record(event):
        ...
        return redirect(url_for('events.calendar_index'))
    if not event_id and not _can_manage_any_event() and not has_permission(PERM_MANAGE_REGION_EVENTS):
        flash('Unauthorized.', 'error')
        return redirect(url_for('events.calendar_index'))
        ...
        if event_id:
            with conn.cursor() as cursor:
                cursor.execute(
                    ...
                    """,
                    (
                        row['title'], row['description'], row['starts_at'], row['ends_at'],
                        row['all_day'], row['location'], row['slurl'], row['region_uuid'],
                        row['organizer_uuid'], row['event_tier'], row['category'], row['status'],
                        row['recurrence_rule'], row['recurrence_until'],
                        row['announce_group_uuid'], row['use_group_chat'], row['use_group_notice'],
                        row['status'], event_id,
                    ),
                )
            conn.commit()
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                (
                    ...
                    row['recurrence_rule'], row['recurrence_until'],
                    row['announce_group_uuid'], row['use_group_chat'], row['use_group_notice'],
                    session['uuid'], session['name'], row['status'],
                ),
            )
    ...
    if event:
        ...
        form = {
            ...
            'use_group_chat': '' if event.get('use_group_chat') is None else ('1' if event['use_group_chat'] else '0'),
            'use_group_notice': '' if event.get('use_group_notice') is None else ('1' if event['use_group_notice'] else '0'),
        }
    ...
    return render_template(
        'events/form.html', event=event, form=form, regions=_list_regions(),
        tiers=EVENT_TIERS, categories=EVENT_CATEGORIES,
        tier_labels=TIER_LABELS, category_labels=CATEGORY_LABELS,
        recurrence_modes=RECURRENCE_MODES,
        weekday_codes=WEEKDAY_CODES, weekday_labels=WEEKDAY_LABELS,
        back_url=_calendar_back_url(),
    )
...
def my_subscriptions():
    ...
    return render_template(
        'events/my_subscriptions.html', subs=subs, follows=follows,
        back_url=_calendar_back_url(),
    )
...
def _feed_response(format_ics=True):
    ...
    if not token_row and not any([
        request.args.get('tier'), request.args.get('category'),
        request.args.get('region'), token,
    ]):
