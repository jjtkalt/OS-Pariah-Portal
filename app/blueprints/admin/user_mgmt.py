...
from app.utils.robust_api import set_user_level, update_robust_email, update_robust_name
...
def gatekeeper_lookup():
    ...
    if query_raw and not has_ppi_access and search_type in ["mac", "host_id"]:
        flash(
            "Unauthorized: You do not have clearance to search by connection PPI.",
            "error",
        )
        return redirect(url_for("user_mgmt.gatekeeper_lookup"))
    ...
    if not query_raw:
        return render_template(
            "admin/lookup.html", results=None, search_type=search_type, query=""
        )
    ...
    try:
        with pariah_conn.cursor() as cursor:
            if search_type == "username":
                ...
                for table in [
                    ...
                    cursor.execute(
                        f"SELECT DISTINCT user_uuid FROM {table} WHERE user_name LIKE %s",
                        (like_query,),
                    )
                ...
                for table in [
                    ...
                    cursor.execute(
                        f"SELECT DISTINCT user_uuid FROM {table} WHERE user_name = %s",
                        (query_raw,),
                    )
                    uuids.update([row["user_uuid"] for row in cursor.fetchall()])
                ...
                cursor.execute(
                    f"SELECT DISTINCT user_uuid FROM {table} WHERE {column} = %s",
                    (query_raw,),
                )
                uuids.update([row["user_uuid"] for row in cursor.fetchall()])
        ...
        with robust_conn.cursor() as r_cursor:
            if search_type == "username":
                r_cursor.execute(
                    "SELECT PrincipalID FROM useraccounts WHERE CONCAT(FirstName, ' ', LastName) LIKE %s",
                    (f"%{query_raw}%",),
                )
                ...
                r_cursor.execute(
                    "SELECT PrincipalID FROM useraccounts WHERE CONCAT(FirstName, ' ', LastName) = %s",
                    (query_raw,),
                )
                uuids.update([row["PrincipalID"] for row in r_cursor.fetchall()])
                ...
                r_cursor.execute(
                    "SELECT PrincipalID FROM useraccounts WHERE PrincipalID = %s",
                    (query_raw,),
                )
            ...
            if uuids:
                ...
                with pariah_conn.cursor() as cursor:
                    ...
                    cursor.execute(
                        f"SELECT user_uuid, MAX(inbound_from) as grid_from, MAX(date_time) as last_seen FROM gatekeeper_from WHERE user_uuid IN ({format_strings}) GROUP BY user_uuid",
                        uuid_tuple,
                    )
                    grid_data = {
                        row["user_uuid"]: {
                            "from": row["grid_from"],
                            "last_seen": format_dt(row["last_seen"]),
                        }
                    ...
                    if has_ppi_access:
                        cursor.execute(
                            f"SELECT user_mac, MAX(date_time) as last_seen FROM gatekeeper_mac WHERE user_uuid IN ({format_strings}) GROUP BY user_mac ORDER BY last_seen DESC",
                            uuid_tuple,
                        )
                        results["macs"] = {
                            r["user_mac"]: format_dt(r["last_seen"])
                            for r in cursor.fetchall()
                            if r["user_mac"]
                        }

                        cursor.execute(
                            f"SELECT user_host_id, MAX(date_time) as last_seen FROM gatekeeper_host_id WHERE user_uuid IN ({format_strings}) GROUP BY user_host_id ORDER BY last_seen DESC",
                            uuid_tuple,
                        )
                        results["host_ids"] = {
                            r["user_host_id"]: format_dt(r["last_seen"])
                            for r in cursor.fetchall()
                            if r["user_host_id"]
                        }
                ...
                with robust_conn.cursor() as r_cursor:
                    r_cursor.execute(
                        f"SELECT PrincipalID, FirstName, LastName, Email, userLevel FROM useraccounts WHERE PrincipalID IN ({format_strings})",
                        uuid_tuple,
                    )
                ...
                with pariah_conn.cursor() as cursor:
                    ...
                    for gk_table in (
                        ...
                        for r in cursor.fetchall():
                            ...
                            if (
                                r["user_name"]
                                and r["user_name"] != "Unknown"
                                and r["user_uuid"] not in robust_data
                            ):
                ...
                grid_fqdn = (
                    str(
                        f"{get_dynamic_config('robust_subdomain')}.{get_dynamic_config('grid_domain')}"
                    )
                    .strip()
                    .lower()
                )
                ...
                for u in uuids:
                    ...
                    uuid_info = dict(
                        sorted(
                            uuid_info.items(),
                            key=lambda item: item[1]["avatar_name"].lower(),
                        )
                    )
...
def user_notes(uuid):
    ...
    if request.method == "POST":
        if not has_permission(PERM_ADD_NOTES):
            flash(
                "Unauthorized: You do not have permission to add staff notes.", "error"
            )
    ...
    with pariah_conn.cursor() as cursor:
        cursor.execute(
            "SELECT * FROM user_notes WHERE user_uuid = %s ORDER BY created_at DESC",
            (uuid,),
        )
    ...
    if notes:
        robust_conn = get_robust_db()
        admin_uuids = list({note["admin_uuid"] for note in notes})
...
def trigger_system_sync_workers(ban_id="Manual/Unknown"):
    ...
    current_app.logger.info(
        f"Triggering system synchronizations for Ban ID {ban_id}..."
    )
    sync_log = "/var/log/os_pariah/sync_workers.log"
    log_out = None
    try:
        os.makedirs(os.path.dirname(sync_log), exist_ok=True)
        # Kept open across subprocess launches so both workers share one log handle.
        log_out = open(sync_log, "ab", buffering=0)  # noqa: SIM115
    except OSError:
        pass
        ...
        with pariah_conn.cursor() as cursor:
            for table in tables:
                ...
                for row in cursor.fetchall():
                    ...
                    ent = row.get("entered")
                    prev = best.get(uid)
                    if prev is None or _entered_ts_newer(ent, prev[0]):
                        best[uid] = (ent, nm)
        return {u: pair[1] for u, pair in best.items()}
    except Exception as e:
        current_app.logger.error(
            f"manage_bans: gatekeeper avatar name lookup failed: {e}"
        )
        return {}


def _collect_ban_evidence(
    pariah_conn, robust_conn, *, seed_uuids, seed_macs, seed_hostids, ban_type
):
    """
    ...
    try:
        with pariah_conn.cursor() as cursor:
            ...
            if ban_type == "mac" and seed_macs:
                ...
                cursor.execute(
                    f"SELECT DISTINCT user_uuid FROM gatekeeper_mac WHERE user_mac IN ({fmt})",
                    tuple(seed_macs),
                )
                linked_uuids.update(
                    [
                ...
                cursor.execute(
                    f"SELECT DISTINCT user_uuid FROM gatekeeper_host_id WHERE user_host_id IN ({fmt})",
                    tuple(seed_hostids),
                )
                linked_uuids.update(
                    [
            ...
            if ban_type == "account" and seed_uuids:
                ...
                cursor.execute(
                    f"SELECT DISTINCT user_mac FROM gatekeeper_mac WHERE user_uuid IN ({fmt_u})",
                    tuple(seed_uuids),
                )
                hop_macs = {
                    r.get("user_mac")
                ...
                cursor.execute(
                    f"SELECT DISTINCT user_host_id FROM gatekeeper_host_id WHERE user_uuid IN ({fmt_u})",
                    tuple(seed_uuids),
                )
                hop_hosts = {
                    r.get("user_host_id")
            ...
            if linked_uuids:
                ...
                cursor.execute(
                    f"SELECT DISTINCT user_uuid, user_mac, user_name FROM gatekeeper_mac WHERE user_uuid IN ({fmt_u})",
                    u_tuple,
                )
                for r in _safe_fetchall(cursor):
                    if r.get("user_mac"):
                ...
                cursor.execute(
                    f"SELECT DISTINCT user_uuid, user_host_id, user_name FROM gatekeeper_host_id WHERE user_uuid IN ({fmt_u})",
                    u_tuple,
                )
                for r in _safe_fetchall(cursor):
                    if r.get("user_host_id"):
                ...
                cursor.execute(
                    f"SELECT user_uuid, MAX(inbound_from) AS inbound_from, MAX(user_name) AS user_name FROM gatekeeper_from WHERE user_uuid IN ({fmt_u}) GROUP BY user_uuid",
                    u_tuple,
                )
                for r in _safe_fetchall(cursor):
                    if r.get("user_uuid"):
            ...
            with robust_conn.cursor() as r_cursor:
                ...
                for r in _safe_fetchall(r_cursor):
                    ...
                    if not pid:
                        continue
                    add_names(
                        pid, f"{r.get('FirstName', '')} {r.get('LastName', '')}".strip()
                    )
    except Exception as e:
    ...
    lines.append(
        f"SeedUuids: {', '.join(sorted(seed_uuids)) if seed_uuids else '(none)'}"
    )
    lines.append(f"SeedMacs: {', '.join(sorted(seed_macs)) if seed_macs else '(none)'}")
    lines.append(
        f"SeedHostIDs: {', '.join(sorted(seed_hostids)) if seed_hostids else '(none)'}"
    )
    lines.append("")
    lines.append(f"LinkedAccounts({len(linked_sorted)}):")
    for u in linked_sorted:
        grid = observed["grids"].get(u) or "Unknown/Local"
        names = sorted(observed["names"].get(u, set()))
        name_str = " | ".join([n for n in names if n]) if names else "Unknown"
        lines.append(f"- {u} :: {name_str} :: GridFrom={grid}")
    lines.append("")
    lines.append(
        f"ObservedMACs({len(macs_sorted)}): {', '.join(macs_sorted) if macs_sorted else '(none)'}"
    )
    lines.append(
        f"ObservedHostIDs({len(host_sorted)}): {', '.join(host_sorted) if host_sorted else '(none)'}"
    )
...
def create_ban():
    if request.method == "GET":
        ...
        return render_template(
            "admin/create_ban.html",
            prep_uuid=prep_uuid,
            prep_mac=prep_mac,
            prep_hostid=prep_hostid,
        )
    ...
    for uid in uuids:
        if is_grid_bot_uuid(uid):
            ...
            log_audit_action(
                "Blocked ban on grid bot", f"Attempted ban on protected UUID {uid}"
            )
            return redirect(url_for("user_mgmt.manage_bans"))

    macs = [m.strip() for m in request.form.get("macs", "").split("\n") if m.strip()]
    hostids = [
        h.strip() for h in request.form.get("hostids", "").split("\n") if h.strip()
    ]
    ...
    try:
        with pariah_conn.cursor() as cursor:
            # 1. Master Record
            cursor.execute(
                "INSERT INTO bans_master (reason, type) VALUES (%s, %s)",
                (reason, ban_type),
            )
            ...
            cursor.execute(
                "UPDATE bans_master SET notes = %s WHERE banid = %s",
                (evidence["notes_text"], ban_id),
            )

            # 2. Cascading Data Inserts
            for mac in macs:
                cursor.execute(
                    "INSERT INTO bans_mac (banid, mac) VALUES (%s, %s)", (ban_id, mac)
                )
            for hostid in hostids:
                cursor.execute(
                    "INSERT INTO bans_host_id (banid, hostid) VALUES (%s, %s)",
                    (ban_id, hostid),
                )
            ...
            for uuid in sorted(
                [u for u in enforced_uuids if u and u not in explicit_uuids]
            ):
                grid_from = evidence["grid_by_uuid"].get(uuid)
                cursor.execute(
        ...
        log_audit_action(
            "Create Ban",
            f"Ban #{ban_id} created (type={ban_type}, level={target_level})",
        )
        flash(
            f"Severity Level {target_level} Ban created and actively enforced successfully.",
            "success",
        )
    except Exception as e:
...
def delete_ban(ban_id):
    ...
    try:
        with pariah_conn.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT uuid FROM bans_related_uuid WHERE banid = %s", (ban_id,)
            )
            related_uuids = [
                row["uuid"] for row in _safe_fetchall(cursor) if row.get("uuid")
            ]
        ...
        log_audit_action(
            "Remove Ban",
            f"Ban #{ban_id} removed; restored {len(banned_uuids)} enforced UUID(s) to level 0.",
        )
        flash(
            f"Ban removed. {len(banned_uuids)} associated avatars have been restored to Level 0.",
            "success",
        )
    except Exception as e:
...
def rename_user(uuid):
    ...
    if is_grid_bot_uuid(uuid):
        ...
        log_audit_action(
            "Blocked rename on grid bot", f"Attempted rename of protected UUID {uuid}"
        )
        return redirect(url_for("user_mgmt.gatekeeper_lookup", type="uuid", q=uuid))
    ...
    if update_robust_name(uuid, new_first, new_last):
        flash(
            f"User successfully renamed to {new_first} {new_last}. (Note: The user may need to clear their viewer cache).",
            "success",
        )
    else:
...
def manage_roles(uuid):
    ...
    with robust_conn.cursor() as r_cursor:
        r_cursor.execute(
            "SELECT FirstName, LastName, userLevel FROM useraccounts WHERE PrincipalID = %s",
            (uuid,),
        )
    ...
    with pariah_conn.cursor() as cursor:
        cursor.execute(
            "SELECT permissions FROM user_rbac WHERE user_uuid = %s", (uuid,)
        )
        row = cursor.fetchone()
        if row:
    ...
    if request.method == "POST":
        ...
        for bit_str in selected_bits:
            try:
                ...
                for _cat, perms in RBAC_SCHEMA.items():
        ...
        try:
            ...
            with pariah_conn.cursor() as cursor:
                if new_bitmask == 0:
                    cursor.execute(
                        "DELETE FROM user_rbac WHERE user_uuid = %s", (uuid,)
                    )
                else:
                    cursor.execute(
                        """
...
def admin_update_email(uuid):
    ...
    if update_robust_email(uuid, new_email):
        log_audit_action(
            "Update Email", f"Changed email to {new_email}", target_uuid=uuid
        )
...
def admin_force_password_reset(uuid):
    ...
    with robust_conn.cursor() as r_cursor:
        r_cursor.execute(
            "SELECT Email FROM useraccounts WHERE PrincipalID = %s", (uuid,)
        )
        account = r_cursor.fetchone()

    if not account or not account["Email"]:
        flash(
            "Cannot send reset link: This user has no email address on file in the grid.",
            "error",
        )
        return redirect(url_for("user_mgmt.gatekeeper_lookup", type="uuid", q=uuid))
    ...
    try:
        token, _expires_at = create_password_reset_token(
            pariah_conn, uuid, ttl_seconds=3600
        )

        send_password_reset_email(account["Email"], token)
        log_audit_action(
            "Force Password Reset", "Forced a password reset", target_uuid=uuid
        )
        flash(
            f"A secure password reset link has been dispatched to {account['Email']}.",
            "success",
        )
    except Exception as e:
        current_app.logger.error(
            f"Failed to generate admin password reset for {uuid}: {e}"
        )
