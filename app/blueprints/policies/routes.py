...
def view_policy(slug):
    ...
    with pariah_conn.cursor() as cursor:
        cursor.execute(
            "SELECT title, body, category, updated_at, requires_login FROM policies WHERE slug = %s",
            (slug,),
        )
...
def manage_policies():
    if not (
        has_permission(PERM_MANAGE_POLICIES)
        or has_permission(PERM_MANAGE_GUIDES)
        or has_permission(PERM_MANAGE_RESOURCES)
    ):
    ...
    with pariah_conn.cursor() as cursor:
        cursor.execute(
            "SELECT slug, title, category, requires_login, updated_at FROM policies ORDER BY category ASC, title ASC"
        )
        all_policies = cursor.fetchall()

    manageable_policies = [
        p for p in all_policies if can_manage_category(p["category"])
    ]

    return render_template(
        "policies/manage.html",
        policies=manageable_policies,
...
def create_policy():
    ...
    if request.method == "POST":
        ...
        if not can_manage_category(category):
            flash(
                f"You do not have permission to create {category} documents.", "error"
            )
        ...
        version_action = (
            request.form.get("version_action") if category == "Policy" else "none"
        )
        ...
        try:
            with pariah_conn.cursor() as cursor:
                cursor.execute(
                    "INSERT INTO policies (slug, title, body, category, requires_login) VALUES (%s, %s, %s, %s, %s)",
                ...
                if version_action in ["minor", "major"]:
                    ...
                    try:
                        ...
                        new_version = (
                            f"{major + 1}.0"
                            if version_action == "major"
                            else f"{major}.{minor + 1}"
                        )
                    except ValueError:
                        new_version = "1.0"
                    ...
                    flash(
                        f"Policy '{title}' created! Global version bumped to {new_version}.",
                        "success",
                    )
                else:
            ...
            flash(
                "Failed to create document. That URL slug might already exist.", "error"
            )
            return redirect(url_for("policies.create_policy"))

    current_version = get_dynamic_config("global_policy_version")
    current_version, next_minor, next_major = _compute_next_policy_versions(
        current_version
    )
    return render_template(
...
def edit_policy(slug):
    ...
    if request.method == "POST":
        ...
        if not can_manage_category(category):
            flash(
                "Unauthorized: You cannot change this document to that category.",
                "error",
            )
            return redirect(url_for("policies.manage_policies"))
        ...
        with pariah_conn.cursor() as cursor:
            cursor.execute(
                "UPDATE policies SET title = %s, body = %s, category = %s, requires_login = %s WHERE slug = %s",
            ...
            if version_action in ["minor", "major"]:
                ...
                log_audit_action(
                    "Policies",
                    f"Updated the global policies to version '{new_version}'",
                )
                flash(
                    f"Policy updated. Global version bumped to {new_version}.",
                    "success",
                )
            else:
    ...
    with pariah_conn.cursor() as cursor:
        cursor.execute(
            "SELECT slug, title, body, category, requires_login FROM policies WHERE slug = %s",
            (slug,),
        )
        policy = cursor.fetchone()
