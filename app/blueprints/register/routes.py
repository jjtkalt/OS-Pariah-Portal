...
from app.utils.notifications import notify_staff_new_app, send_verification_email
...
def register():
    ...
    if not new_uuid:
        flash(
            "Registration failed. The avatar name might already be taken or the grid is offline.",
            "error",
        )
        return redirect(url_for("register.register"))
    ...
    try:
        ...
        with pariah_conn.cursor() as cursor:
            if reg["requires_approval"]:
                ...
                cursor.execute(
                    "UPDATE pending_registrations SET status = 'pending_approval' WHERE user_uuid = %s",
                    (user_uuid,),
                )
                flash(
                    "Email verified! Your account is now awaiting staff approval. You will receive an email once approved.",
                    "success",
                )

                # Ping staff NOW, because we know they are a real human with a working email!
                notify_staff_new_app(
                    first_name,
                    last_name,
                    user_uuid,
                    reg["inviter"],
                ...
                flash(
                    "Email verified! Your account has been activated and you can now log in to the grid.",
                    "success",
                )
