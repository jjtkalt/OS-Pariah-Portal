...
@patch('app.utils.maintenance.os.listdir')
@patch('app.utils.maintenance.os.path.getmtime')
@patch('app.utils.maintenance.os.remove')
@patch('app.utils.maintenance.os.path.exists')
@patch('app.utils.maintenance.get_dynamic_config')
def test_clean_texture_cache(mock_get_config, mock_exists, mock_remove, mock_getmtime, mock_listdir, app):
    """Test that the cleanup routine correctly identifies and deletes old files."""
    from app.utils.maintenance import clean_texture_cache
    ...
    with app.app_context():
        ...
        # Run cleanup
        deleted = clean_texture_cache(days_old=30)
...
def test_texture_gallery_access(mock_get_db, client):
    ...
    # Mock the database cursor and return data
    mock_conn = MagicMock()
    ...
    # Assume we have a way to fake an admin session (adjust to match your actual fixture/auth mocking)
...
def test_serve_texture_cached(mock_send_dir, mock_get_config, mock_exists, client):
    """Test that an already cached texture skips OpenCV and serves directly."""
    mock_get_config.side_effect = lambda k: '/fake/dir' if k == 'texture_cache_path' else None
...
@patch('app.blueprints.admin.routes.get_dynamic_config')
@patch('app.blueprints.admin.routes.send_from_directory')
def test_serve_texture_unzipped_and_converted(mock_send_dir, mock_get_config, mock_exists, mock_access, mock_gzip, mock_cv2, client):
    ...
    assert response.status_code == 200
    # Ensure OpenCV attempted to save the result
