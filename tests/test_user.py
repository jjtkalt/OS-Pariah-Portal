...
@patch('app.utils.password_resets.secrets.token_urlsafe', return_value='fixed-reset-token')
def test_password_reset_workflow_mints_and_burns_token(
    ...
    mock_get_robust.return_value.cursor.return_value.__enter__.return_value = robust_cursor
    robust_cursor.fetchone.return_value = {'PrincipalID': 'fake-user-uuid', 'Email': 'user@example.com'}

    # --- Step 1: request a reset token ---
    db_cursor.reset_mock()
    resp = client.post('/auth/forgot', data={
        'first_name': 'Test',
        'last_name': 'Avatar',
    ...
    assert sqls[2] == "INSERT INTO password_resets (token, user_uuid, expires_at) VALUES (%s, %s, %s)"
    ...
    resp2 = client.post('/auth/reset/fixed-reset-token', data={
    ...
    assert sqls2[0].startswith("DELETE FROM password_resets WHERE expires_at <=")  # opportunistic purge
