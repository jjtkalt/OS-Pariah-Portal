...
from app.utils.schema import PERM_MANAGE_EVENTS, PERM_APPROVE_EVENTS
...
def test_group_occurrences_by_local_date(app):
    ...
    with app.app_context():
        ...
        occs = (
            expand_event_occurrences(ev, datetime(2026, 5, 1), datetime(2026, 6, 1))
...
def test_feed_ics_public(client):
    ...
    assert b'VCALENDAR' in response.data or response.content_type.startswith('text/calendar')
