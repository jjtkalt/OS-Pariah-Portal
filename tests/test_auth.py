...
def generate_mock_hash(password, salt):
    ...
    return hashlib.md5(f"{pass_md5}:{salt}".encode()).hexdigest()
...
def test_successful_login(mock_get_robust, client):
    ...
    response = client.post(
        "/auth/login",
        data={"first_name": "Test", "last_name": "Avatar", "password": password},
...
def test_banned_user_blocked(mock_get_robust, client):
    ...
    response = client.post(
        "/auth/login",
        data={"first_name": "Bad", "last_name": "Guy", "password": password},
        follow_redirects=True,
    )

    assert (
        b"Your account is currently locked, pending approval, or banned."
        in response.data
    )
