...
def test_manage_roles_self_edit_blocked(client, db_cursor):
    ...
    response = client.post(
        "/admin/users/admin-uuid-123/roles",
        data={"permissions": str(PERM_MANAGE_ROLES)},
        follow_redirects=True,
    )

    # 3. Assert the bouncer stopped them before touching the DB
    assert (
        b"Security Violation: You cannot modify your own permissions" in response.data
    )
...
def test_manage_roles_preserves_super_bits(mock_get_robust, client, db_cursor):
    ...
    client.post(
        "/admin/users/target-uuid-456/roles",
    ...
    for call in db_cursor.execute.call_args_list:
        ...
        if "INSERT INTO user_rbac" in query and args == (
            "target-uuid-456",
            expected_bitmask,
        ):
    ...
    assert saved_correctly, (
        f"Failed to preserve super_only bits. Expected bitmask: {expected_bitmask}"
    )
...
def test_ppi_search_blocked_without_permission(client):
    ...
    response = client.get(
        "/admin/users/lookup?type=mac&q=00:11:22:33:44:55", follow_redirects=True
    )

    # Assert the request was intercepted
    assert (
        b"Unauthorized: You do not have clearance to search by connection PPI"
        in response.data
    )
