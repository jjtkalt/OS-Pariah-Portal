...
def test_successful_registration(
    ...
    mock_create_robust.assert_called_once_with(
        "Test", "Avatar", "SecurePassword123!", "test@example.com"
    )
    ...
    assert any("INSERT INTO pending_registrations" in q for q in sql_queries_run), (
        "Did not attempt to insert registration into DB."
    )
    assert any("INSERT INTO user_notes" in q for q in sql_queries_run), (
        "Did not copy registration application to user notes."
    )

    note_insert = next(
        c
        for c in db_cursor.execute.call_args_list
        if "INSERT INTO user_notes" in c[0][0]
    )
